...
class TEMA_BB_RSI_Hyperopt01(IHyperOpt):
    """
    ...
    def buy_strategy_generator(params: Dict[str, Any]) -> Callable:
        ...
        def populate_buy_trend(dataframe: DataFrame, metadata: dict) -> DataFrame:
            ...
            if params.get('rsi-enabled'):
                conditions.append(qtpylib.crossed_above(dataframe['rsi'], int(params['buy-rsi-value'])))
            ...
            if 'buy-trigger' in params:
                for std in range(1, 5):
                    if params['buy-trigger'] == f'buy_bb_lower{std}':
                        conditions.append(dataframe['tema'] <= dataframe[f'bb_lowerband{std}'])
                    if params['buy-trigger'] == f'buy_bb_middle{std}':
                        conditions.append(dataframe['tema'] <= dataframe[f'bb_middleband{std}'])
                    if params['buy-trigger'] == f'buy_bb_upper{std}':
                        conditions.append(dataframe['tema'] <= dataframe[f'bb_upperband{std}'])

            conditions.append(dataframe['tema'] > dataframe['tema'].shift(params['buy-shift-value']))
    ...
    def indicator_space() -> List[Dimension]:
        ...
        return [
            ...
            Integer(1, 2, name='buy-shift-value'),
    ...
    def sell_strategy_generator(params: Dict[str, Any]) -> Callable:
        ...
        def populate_sell_trend(dataframe: DataFrame, metadata: dict) -> DataFrame:
            ...
            if params.get('sell-rsi-enabled'):
                conditions.append(qtpylib.crossed_above(dataframe['rsi'], int(params['sell-rsi-value'])))
            ...
            conditions.append(dataframe['tema'] < dataframe['tema'].shift(params['sell-shift-value']))
    ...
    def sell_indicator_space() -> List[Dimension]:
        ...
        return [
            ...
            Integer(1, 2, name='sell-shift-value'),
